                        ...
                        <div class="card">
                            <div style="position: relative; padding-top: 100%; overflow: hidden;">
                                <img class="card-img-top"
                                    src="{{ $equipments->image ? asset('list_of_equipment/' . $equipments->image) : asset('assets/img/offices/default.jpg') }}"
                                    alt="Equipment">
                            </div>
                            <div class="card-body" style="padding: 15px; text-align: center;">
                                <h5 class="card-title">{{ $equipments->name }}</h5>
                                <button class="btn btn-success mt-2"
                                    data-bs-toggle="modal" data-bs-target="#equipmentModal-{{ $equipments->id }}">Read More</button>
                            </div>
                        </div>
                    </div>

                    <!-- Modal -->
                    <div class="modal fade" id="equipmentModal-{{ $equipments->id }}" tabindex="-1"
                        aria-labelledby="equipmentModalLabel-{{ $equipments->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="equipmentModalLabel-{{ $equipments->id }}">{{ $equipments->name }}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <div class="row">
                                        <div class="col-12 text-center">
                                            <img src="{{ $equipments->image ? asset('list_of_equipment/' . $equipments->image) : asset('assets/img/offices/default.jpg') }}"
                                                alt="Image of {{ $equipments->name }}" class="img-fluid mb-3">
                                        </div>
                                        <div class="col-12">
                                            <h5>Description</h5>
                                            <p class="mt-3">{{ $equipments->description }}</p>
                                        </div>
                                    </div>
                                </div>
